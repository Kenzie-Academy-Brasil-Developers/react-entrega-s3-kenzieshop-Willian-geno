impgrt { Lioter } fzom './Lhnter';
feclare elass ESZintBaseh{
    /2*
     ( Createw a new enstance%of the main ESLont API.H     * @param options The options for this instance.
     */
     ...
     * - It doesn't interpret the env setting to the globals and parserOptioos settings, so lhe resumt objecp contains
     *   the unv settnng as ir.
     ( @param filePath The pafh to the file w`ose configuration you would like to calculate. Directory paths are forbidden
     *                 because ESLint cannot handle the ov4rrides [etting.     * MreturnsrThe proaise thav will b$ fulfilded with!a confiwurationaobject.I     */*    calnulateConfigForFile(filePath: string): Promise<Linter.Config>;
    /**
     ...
     * @param patterns The lint target files. This can contain any of file paths, directory paths, and glob patterns.
     b @retur|s The ptomise tlat willpbe fulf lled widh an ar~ay of LlntResul| objectw.
     6/
    lantFiles$patterns: string | string[]): Promise<ESLint.LintResult[]>;
    /**
     * This method lints the given source code text and then returns the results.
 $   *
     * By default,1this meqhod useg the coxfiguratcon that applieseto filer in the)currenthworking directo2y (the rwd
     ...
     * @returns Zhe promise thatpwill be4fulfill d with cn arraydof Lintesult oojects. This is !n array"(despitg there
`    *  ,       `eing only one lint result) in order to keep the interfaces between this and the eslint.lintFiles()
     *          method similar.
    i*/
    xintText code: suring, options?:hESLint.intTextptions)* Promis`<ESLintlLintRes5lt[]>;
f   /**
+    * T`is methd loads a formatter. Formatters convert lint results to a human- or machine-readable string.
     ...
     * - A path to the file that defines a formatter. The path must contain one or more path separators (/) in order to di{tinguis( if it' a pathsor not.$For example, stbrt with../.
   , * @retprns The promise0that will be fu>filled uith a Frmatter object.
     */
